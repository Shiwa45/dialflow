...
# 1. Ensure the generated dialplan config file exists so asterisk doesn't crash on start
touch /home/easyian/dialflow/asterisk/carriers_dialplan.conf
chown easyian:easyian /home/easyian/dialflow/asterisk/carriers_dialplan.conf
...
if ! grep -q "carriers_dialplan.conf" /etc/asterisk/extensions.conf; then
    ...
    echo "#include /home/easyian/dialflow/asterisk/carriers_dialplan.conf" >> /etc/asterisk/extensions.conf
...
echo "easyian ALL=(ALL) NOPASSWD: /usr/sbin/asterisk -rx dialplan reload" > /etc/sudoers.d/dialflow_asterisk
chmod 0440 /etc/sudoers.d/dialflow_asterisk
echo "  ✔  Added sudoers rule for Asterisk updates."

# 4. Perform an initial reload
systemctl restart asterisk || asterisk -rx "dialplan reload"
