...
class Carrier(TimestampedModel):
    ...
    def rebuild_asterisk_dialplan(cls):
        ...
        file_path = '/etc/asterisk/carriers_dialplan.conf'
        ...
        try:
            with open(file_path, 'w') as f:
                ...
                for carrier in carriers:
                    ...
                    f.write(f"{rendered_code}\n\n")
        except OSError as e:
            raise AsteriskSyncError(f'Failed to write {file_path}: {e}') from e

        # Reload dialplan using sudo (requires sudoers configuration for user).
        # Use absolute asterisk path to match setup_dialplan.sh sudoers rule.
        try:
            subprocess.run(
    ...
    def _normalize_carrier_dialplan_target(dialplan_code, endpoint_id):
        """
        Keep default trunk dial lines aligned with the current carrier endpoint id.
        Fixes cases where a carrier was renamed but dialplan still dials old endpoint.
        """
        if not dialplan_code:
            return dialplan_code

        pattern = r'(Dial\(\s*PJSIP/\$\{EXTEN\}@)(carrier_[A-Za-z0-9_]+)'

        def repl(match):
            return f'{match.group(1)}{endpoint_id}'

        return re.sub(pattern, repl, dialplan_code)
