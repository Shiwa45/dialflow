...
class AsteriskSyncError(RuntimeError):
    """Raised when syncing dialplan/PJSIP changes to Asterisk fails."""
    pass
...
class Carrier(TimestampedModel):
    ...
    @property
    def dialplan_context(self):
        """First explicit context from dialplan_code, else a predictable fallback."""
        import re

        if self.dialplan_code:
            match = re.search(r'^\s*\[([^\]]+)\]', self.dialplan_code, re.MULTILINE)
            if match:
                return match.group(1).strip()
        return f'{self.endpoint_id}-out'

    def sync_to_asterisk(self, previous_endpoint_id=None):
        """
        ...
        # ps_aors — static contact for carrier host
        ...
        # If carrier name changed, endpoint_id changes as well.
        # Clean stale realtime rows so Asterisk does not keep old "Unavailable" trunks.
        if previous_endpoint_id and previous_endpoint_id != ep_id:
            PjsipEndpoint.objects.filter(id=previous_endpoint_id).delete()
            PjsipAuth.objects.filter(id=previous_endpoint_id).delete()
            PjsipAor.objects.filter(id=previous_endpoint_id).delete()
            from django.db import connection
            with connection.cursor() as cur:
                cur.execute("DELETE FROM ps_contacts WHERE endpoint = %s", [previous_endpoint_id])
    ...
    def rebuild_asterisk_dialplan(cls):
        """
        Rebuilds the carriers_dialplan.conf inside WSL2 Asterisk and reloads dialplan.
        On Windows, Django cannot write to /etc/asterisk directly (it lives in WSL2).
        We pipe content via 'wsl -u root tee' to write it safely into WSL2.
        """
        import subprocess
        import sys

        wsl_file_path = '/etc/asterisk/carriers_dialplan.conf'
        carriers = cls.objects.filter(is_active=True).exclude(dialplan_code='')

        lines = [
            "; Auto-generated by DialFlow Pro\n",
            "; Do not edit manually. Edit the Carrier 'dialplan_code' via Web UI.\n\n",
        ]
        for carrier in carriers:
            lines.append(f"; Carrier: {carrier.name}\n")
            rendered_code = cls._normalize_carrier_dialplan_target(
                carrier.dialplan_code,
                carrier.endpoint_id,
            )
        ...
        try:
            ...
            raise AsteriskSyncError(f'Failed to write {wsl_file_path}: {e}') from e

        try:
            if is_windows:
                subprocess.run(
                    ['wsl', '-u', 'root', '/usr/sbin/asterisk', '-rx', 'module reload pbx_config.so'],
            ...
            logger.error('Dialplan reload failed: %s', detail)
            raise AsteriskSyncError(f'Dialplan reload failed: {detail}') from e
        except FileNotFoundError as e:
            logger.error('Dialplan reload command not found: %s', e)
            raise AsteriskSyncError(f'Dialplan reload command not found: {e}') from e

    @staticmethod
    def _normalize_carrier_dialplan_target(dialplan_code, endpoint_id):
        """
        Rewrite every ``Dial(PJSIP/<anything>@carrier_<old>)`` line so the
        endpoint name matches the current endpoint_id.

        Handles all variable forms: ${EXTEN}, ${NUMBER}, ${OUTNUM}, etc.
        Also corrects hard-coded numbers in manual dialplan entries.
        """
        if not dialplan_code:
            return dialplan_code

        # Match Dial(PJSIP/<any-value>@carrier_<old-name>)
        # and replace only the endpoint portion after '@'
        pattern = r'(Dial\(\s*PJSIP/[^@]+@)(carrier_[A-Za-z0-9_]+)'

        def repl(match):
            return f'{match.group(1)}{endpoint_id}'

        return re.sub(pattern, repl, dialplan_code, flags=re.IGNORECASE)
...
class PjsipEndpoint(models.Model):
    ...
    dtls_auto_generate_cert   = models.CharField(max_length=10, default='no')
    rtcp_mux                  = models.CharField(max_length=10, default='no')
    ...
    def sync_to_asterisk(self):
        ...
        PjsipEndpoint.objects.update_or_create(
            ...
            defaults={
                ...
                'dtls_auto_generate_cert':  'yes' if is_webrtc else 'no',
                'rtcp_mux':                 'yes' if is_webrtc else 'no',
                'dtmf_mode':                'rfc4733',
                'send_rpid':                'yes',
            }
        )
