...
def _carrier_payloads():
    return list(
        Carrier.objects.select_related('asterisk_server').values(
            'id',
            'name',
            'asterisk_server',
            'protocol',
...
def server_list(request):
    ...
    return render(request, 'telephony/server_list.html', {
        ...
        'carrier_payloads': _carrier_payloads(),
...
def carrier_create(request):
    ...
    try:
        carrier = Carrier.objects.create(
            ...
            dialplan_code=data.get('dialplan_code', ''),
            is_active=data.get('is_active', True) in (True, 'true', '1', 'on'),
        )
...
def carrier_edit(request, pk):
    ...
    for field in ['name', 'description', 'protocol', 'host', 'username',
                  'password', 'caller_id', 'dial_prefix', 'dialplan_code']:
        if field in data:
